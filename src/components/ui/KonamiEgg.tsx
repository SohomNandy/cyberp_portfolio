...
export default function KonamiEgg() {
  ...
  const [phase, setPhase]         = useState(0); // 0=hidden 1=glitch 2=message 3=fadeout
  ...
  const activate = () => {
    if (triggered) return;
    setTriggered(true);
    (window as any).__playSound?.("glitch");

    // Phase 1: glitch flash
    setPhase(1);
    setTimeout(() => setPhase(2), 400);
    setTimeout(() => {
      (window as any).__playSound?.("boot");
    }, 500);
      ...
      {phase === 2 && (
        <div className="relative text-center px-8">
          ...
          <div className="relative">
            <div
              className="font-display font-black text-2xl sm:text-4xl text-accent tracking-widest"
              style={{
                textShadow: "0 0 30px rgba(0,255,136,0.8), 0 0 60px rgba(0,255,136,0.4)",
                animation: "rgbShift 0.5s ease-in-out infinite",
              }}
            >
            ...
            {/* Red offset */}
            <div
              className="absolute inset-0 font-display font-black text-2xl sm:text-4xl tracking-widest"
          ...
          <div className="mt-8 flex justify-center gap-3 flex-wrap">
            {["GNN ARCHITECT","LLM WHISPERER","THREAT HUNTER","KOLKATA REPRESENT"].map(tag => (
              <span key={tag}
                className="px-3 py-1 font-label text-xs border border-accent text-accent"
                style={{
